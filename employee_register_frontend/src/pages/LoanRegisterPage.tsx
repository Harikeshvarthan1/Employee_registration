...
import { getAllLoans, getActiveLoans, registerLoan, updateLoanStatus, deleteLoan, updateLoan } from '../apis/loanRegistrationApi';
...
const LoanRegisterPage: React.FC = () => {
  ...
  // State to track if we're adding to an existing loan
  const [existingLoan, setExistingLoan] = useState<LoanRegistration | null>(null);
  ...
  // Check for existing active loan when employee is selected
  useEffect(() => {
    const checkForExistingLoan = () => {
      if (loanForm.employeeId > 0) {
        // Find if the employee already has an active loan
        const existing = activeLoans.find(
          loan => loan.employeeId === loanForm.employeeId && loan.status === 'active'
        );
    ...
    checkForExistingLoan();
  }, [loanForm.employeeId, activeLoans]);
  ...
  // Toggle adding to existing loan
  const toggleAddToExisting = () => {
    setIsAddingToExisting(!isAddingToExisting);
  };

  // Submit the loan registration form - FIXED VERSION
  const handleSubmitLoan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If adding to existing loan and user has confirmed
      if (existingLoan && isAddingToExisting) {
        // Calculate new total amount
        const newTotalAmount = existingLoan.loanAmount + loanForm.loanAmount;
        
        // Create updated loan object
        const updatedLoan = {
          ...existingLoan,
          loanAmount: newTotalAmount,
          // Append the new reason to the existing reason
          reason: `${existingLoan.reason} + ADDITIONAL LOAN (${formatDate(loanForm.loanDate)}): ${loanForm.reason}`
        };
        
        console.log('Updating existing loan:', updatedLoan);
        result = await updateLoan(existingLoan.loanId || 0, updatedLoan);
        console.log('Loan update response:', result);
        
        successMsg = `Added ${formatCurrency(loanForm.loanAmount)} to existing loan. New total: ${formatCurrency(newTotalAmount)}`;
      } else {
        // Create a new loan
        const newLoan = {
        ...
        console.log('Registering new loan:', newLoan);
        result = await registerLoan(newLoan);
        console.log('Loan registration response:', result);
        
        successMsg = 'New loan registered successfully!';
      }
  ...
  const handleDeleteLoan = async () => {
    ...
    try {
      console.log(`Deleting loan with ID: ${loanToDelete.loanId}`);
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      console.log(`Updating loan ${loanToUpdateStatus.loanId} status to: ${newStatus}`);
      
      // Send the new status directly
  ...
  return (
    <div className="loan-register-page">
      <main className="loan-register-content">
        ...
        {viewMode === 'form' && !showLoading && (
          <div className="loan-form-section" ref={formRef}>
            <div className="loan-form-card">
              <div className="card-header">
                <h2>
                  <i className="bi bi-file-earmark-plus"></i>
                  {existingLoan ? 'Add to Existing Loan' : 'Register New Loan'}
                </h2>
              </div>
              
              {/* Existing loan notification */}
              {existingLoan && (
                <div className={`existing-loan-alert ${isAddingToExisting ? 'adding' : 'creating'}`}>
                  <div className="alert-icon">
                    <i className={`bi ${isAddingToExisting ? 'bi-plus-circle-fill' : 'bi-exclamation-triangle-fill'}`}></i>
                  </div>
                  <div className="alert-content">
                    <h4>{getEmployeeName(existingLoan.employeeId)} already has an active loan</h4>
                    <p>Current loan amount: <strong>{formatCurrency(existingLoan.loanAmount)}</strong></p>
                    <p>
                      {isAddingToExisting 
                        ? "You are adding to this employee's existing loan balance." 
                        : "You are creating a separate loan for this employee."}
                    </p>
                  </div>
                  <div className="alert-action">
                    ...
                    >
                      {isAddingToExisting 
                        ? 'Create Separate Loan Instead' 
                        : 'Add to Existing Loan Instead'}
                    </button>
                  </div>
                </div>
              )}
              ...
              <div className="card-body">
                <form onSubmit={handleSubmitLoan}>
                  <div className="form-grid">
                    ...
                    <div className="form-group">
                      ...
                      {loanForm.loanAmount > 0 && (
                        <div className="amount-preview">
                          ...
                          {existingLoan && isAddingToExisting && (
                            <div className="new-total">
                              New total will be: {formatCurrency(existingLoan.loanAmount + loanForm.loanAmount)}
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    {/* Status selection - only show if creating a new loan */}
                    {(!existingLoan || !isAddingToExisting) && (
                      <div className="form-group">
                    ...
                    <div className="form-group full-width">
                      ...
                      {existingLoan && isAddingToExisting && (
                        <div className="help-text">
                          This reason will be appended to the existing loan reason.
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {/* Loan details view */}
        {viewMode === 'detail' && selectedLoan && !showLoading && (
              ...
              <div className="card-body">
                  ...
                  <div className="loan-info-section">
                    ...
                    <div className="loan-detail-row">
                      ...
                      <div className="detail-value reason-text">
                        {selectedLoan.reason}
                        {selectedLoan.reason.includes('ADDITIONAL LOAN') && (
                          <div className="consolidated-loan-note">
                            <i className="bi bi-info-circle"></i>
                            This is a consolidated loan with multiple disbursements.
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="loan-detail-actions">
                  <button 
                    ...
                    onClick={() => {
                      handleRegisterLoan();
                      // Pre-select the employee in the form
                      setLoanForm(prev => ({
                        ...prev,
        ...
        {viewMode === 'list' && !showLoading && (
          <>
            ...
            <div className="loans-list-section">
              ...
              {/* Empty state */}
              {!showLoading && filteredLoans.length === 0 && (
              ...
              {!showLoading && filteredLoans.length > 0 && (
                <div className="loans-grid">
                  {filteredLoans.map(loan => {
                    const employee = employees.find(emp => emp.id === loan.employeeId);
                    const isConsolidated = loan.reason.includes('ADDITIONAL LOAN');
                    
                    return (
                      <div key={loan.loanId} className={`loan-card ${loan.status} ${isConsolidated ? 'consolidated' : ''}`}>
                        {isConsolidated && (
                          <div className="consolidated-badge">
                        ...
                        <div className="loan-body">
                          ...
                          <div className="loan-reason">
                            ...
                            <div className="reason-text">
                              {isConsolidated 
                                ? "Multiple loan disbursements - see details" 
